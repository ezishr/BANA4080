...
rescale <- function(x, digits = 2, na.rm = TRUE, quiet = FALSE){
  if(!is.atomic(x) | !is.numeric(x)) {
    stop("Your `x` must be numeric", .call = FALSE)
  }
  ...
  if(!na.rm) {
    warning(paste("There are",
                  missing_val,
                  "NA values in your x with na.rm = TRUE",
                  .call = FALSE))
  }
  
  if(na.rm) {
    if(!quiet) {
      message(paste(missing_val, "NAs were removed"))
    }
  }
...
path <- here("Module 6/lesson-6c-student-files")
fx_path <- here(path, "my-functions", "custom-stat-functions.R")
source(fx_path)
ls()
