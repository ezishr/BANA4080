...
system.time({
  for (i in seq_along(means)) {
    ...
    out <- c(out, gen_data) # 	•	If out already contains some values, this line will append the contents of gen_data to out.
  }
})

out <- vector("list", length(means)) #vector() is a function used to create a vector of a specified type and length.
system.time({
  for (i in seq_along(means)) {
...
library(here)
path <- here("Module 6/")
list.files(here(path, "data"), pattern = "Month")
```
...
(first_df <- list.files(here(path, "data"), pattern = "Month")[1])

# create path and import this single file
df <- readr::read_csv(here(path, "data", first_df))
...
```{r slide-9c}
# fill in these gaps
for(i in list.files(here(path, "data"), pattern="Month-")) {
  # 1: import data
  df <- readr::read_csv(here(path,"data",i))
...
```{r slide-14a}
# data files
(data_files <- list.files(here("data"), pattern = "Month"))
...
for(i in list.files(here("data"), pattern = "Month")) {
  # steps to import each data set
  df <- readr::read_csv(here("data", i))
  new_name <- stringr::str_sub(i, end = -5)
  assign(new_name, df)
  rm(df)
}
